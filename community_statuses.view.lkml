view: community_statuses {
  derived_table: {
    sql: select community_name, intake_community_id, count (distinct family_id) Number_of_Families, count(distinct recipient_id) Number_of_Recipients, count(distinct survey_phase_id) survey_phases_completed, count(distinct survey_families) Surveys_completed
      from (
